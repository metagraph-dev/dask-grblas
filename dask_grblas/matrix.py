...
class Matrix(BaseType):
    ...
    def from_delayed(cls, matrix, dtype, nrows, ncols, *, name=None):
        if not isinstance(matrix, Delayed):
            raise TypeError('Value is not a dask delayed object.  Please use dask.delayed to create a grblas.Matrix')
    ...
    @classmethod
    def from_values(cls, rows, columns, values, *, nrows=None, ncols=None, dup_op=None, dtype=None, chunks=None, name=None):
        matrix = gb.Matrix.from_values(rows, columns, values, nrows=nrows, ncols=ncols, dup_op=dup_op, dtype=dtype)
    ...
    def new(cls, dtype, nrows=0, ncols=0, *, name=None):
        ...
        return cls.from_delayed(delayed(matrix), matrix.dtype, matrix.nrows, matrix.ncols, name=name)
