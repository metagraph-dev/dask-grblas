...
def row_stack(seq):
    for i, x in enumerate(seq):
        if not isinstance(x, (Vector, Matrix)):
            raise TypeError(f'Bad type of value in position {i}.  God {type(x)}, expected Vector or Matrix')
...
def concat_vectors(seq):
    ...
    for i, x in enumerate(seq):
        if not isinstance(x, Vector):
            raise TypeError(f'Bad type of value in position {i}.  God {type(x)}, expected Vector')
