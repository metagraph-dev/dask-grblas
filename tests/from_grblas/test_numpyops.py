...
def test_npunary():
    ...
    for gb_input, np_input in data:
        for unary_name in sorted(npunary._unary_names):
            ...
            compare = match.reduce(monoid.land).value
            if not compare:
...
def test_npbinary():
    ...
    data = [
        [
            [
                dgb.Vector.from_values(index, values1),
                dgb.Vector.from_values(index, values2),
            ],
            [
        ...
        [
            [
                dgb.Vector.from_values([0, 1, 2, 3], [True, False, True, False]),
                dgb.Vector.from_values([0, 1, 2, 3], [True, True, False, False]),
            ],
            [
...
def test_npsemiring():
    for monoid_name, binary_name in itertools.product(
        ...
        semiring = ops.Semiring.register_anonymous(monoid, binary, name)
