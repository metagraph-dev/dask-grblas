...
def compare(
    func, gb_args, dgb_args, gb_kwargs={}, dgb_kwargs={}, *, compute=None, errors=False
):
    if compute is None:
        compute = not errors
    ...
    dgb_objects.extend(
        x for x in dgb_kwargs.values() if isinstance(x, dgb.base.BaseType)
    )
    ...
    if compute:
        ...
        try:
            ...
            if not errors:
                raise
            dgb_result = exc
    assert type(gb_result) == type(
        dgb_result
    ), f"{type(gb_result)} - {type(dgb_result)}"
    if isinstance(gb_result, gb.base.BaseType):
        assert (
            gb_result.dtype == dgb_result.dtype
        ), f"{gb_result.dtype} - {dgb_result.dtype}"
